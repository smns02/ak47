...
LOCAL_KEY_FILE = ".license.json"  # Key ကို သိမ်းထားမယ့် ဖိုင်နာမည်
...
def get_local_key():
    """ဖုန်းထဲက Key သိမ်းထားတဲ့ဖိုင်ကို ဖတ်ခြင်း"""
...
def save_local_key(key, exp_date):
    """Key နှင့် Expire Date ကို ဖုန်းထဲ သိမ်းခြင်း"""
    try:
        with open(LOCAL_KEY_FILE, "w") as f:
            json.dump({"key": key, "exp": exp_date}, f)
    except: pass

def check_approval(user_key):
    # ၁။ အရင်ဆုံး Local မှာ Key ရှိလား၊ ရှိရင် ရက်မကျော်သေးဘူးလား အရင်စစ်မယ်
    local_data = get_local_key()
    if local_data and local_data.get("key") == user_key:
        today = datetime.now().date()
        try:
            exp = datetime.strptime(local_data["exp"], "%Y-%m-%d").date()
            if today <= exp:
                return True, local_data["exp"] # Local ကနေတင် ကျော်ပေးလိုက်မယ်
        except: pass

    # ၂။ Local မှာမရှိရင် သို့မဟုတ် ရက်ကျော်နေရင် Online ကနေ ပြန်စစ်မယ်
    try:
            ...
            for line in lines:
                if "|" in line:
                    ...
                    if app_key.strip() == user_key:
                        ...
                        if today <= exp:
                            # Online ကကျရင် Local မှာ သိမ်းထားလိုက်မယ်
                            save_local_key(user_key, exp_date.strip())
